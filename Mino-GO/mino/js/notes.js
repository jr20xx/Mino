$(() => {
    ...
    $.ajax({
        ...
        error: () => {
            Swal.fire({
                icon: 'error',
                title: "Error retrieving user's notes!",
                toast: true,
                position: 'bottom',
                timer: 3000,
    ...
    note_writer_form.submit((e) => {
        ...
        if (noteTitle.length > 0 && noteBody.length > 0) {
            $.ajax({
                ...
                success: (data) => {
                    addNote(data.ID, data.TITLE, data.BODY, data.TIME_STAMP);
                ...
                $.ajax({
                    ...
                    success: (result) => {
                        if (result.status == 200)
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: "Your current session can not be finished",
                                toast: true,
                                position: 'bottom',
                                timer: 3000,
                                timerProgressBar: true,
            ...
            case 'save_changes':
                ...
                $.ajax({
                    ...
                    success: (data) => {
                        removeActiveNote();
                        disableSaveButton();
                        let rawTimestamp = data.TIME_STAMP;
                        addNote(data.ID, data.TITLE, data.BODY, rawTimestamp, true);
                ...
                $.ajax({
                    ...
                    success: (response) => {
                        if (response.status == 200) {
                            ...
                            Swal.fire({
                                icon: 'error',
                                title: "The note could not be removed",
                                toast: true,
                                position: 'bottom',
                                timer: 3000,
                                timerProgressBar: true,
